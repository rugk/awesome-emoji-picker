 ...
 * @property {boolean} [allowDismiss=true] set to false to disallow dismissing
 * the message. This likely makes no sense for any tip, so the default is true.
 * @property {boolean|number} [requireDismiss=false] show the message, if it is
 ...
 * @property {number|null} [maximumDismiss=null] hides the message, if it
 * has been dismissed x times.
 * @property {number} [requiredTriggers=10] require some displays ("triggers")
 * of (any) add-on page before showing tip. This is effectively just a minimum
 * limit, so it is not shown too "early".
 * @property {Object.<string, number>} [showInContext] a key-value object with
 ...
 * @property {Object.<string, number>} [maximumInContest] a key-value object with
 * context -> num to only show the tip in a specific context at most for the
 * given number of times. See {@link RandomTips.setContext}.
 * @property {boolean|number} [randomizeDisplay=false] Randomizes the display with a
